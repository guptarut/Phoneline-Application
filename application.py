...
def process_event_history(log: Dict[str, List[Dict]],
    ...
    function, every time a new month is detected for the current event you are
    extracting.
    ...
    for event_data in log['events']:
        data_time = datetime.datetime.strptime(event_data['time'],
                                               "%Y-%m-%d %H:%M:%S")
        if data_time.month != billing_month or data_time.year != billing_year:
            billing_month = data_time.month
            billing_year = data_time.year
            new_month(customer_list, billing_month, billing_year)

        if event_data["type"] == "call":
            call = Call(event_data['src_number'], event_data['dst_number'],
                        data_time, event_data['duration'],
            ...
            receiver = find_customer_by_number(event_data["dst_number"],
                                               customer_list)
            if (receiver is not None) and (caller is not None):
