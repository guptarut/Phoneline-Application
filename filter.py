...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if (filter_string == '') or (not filter_string.isnumeric()) or \
                (int(filter_string) not in ids):
            return data
        else:
            customer = customers[ids.index(int(filter_string))]
            phone_numbers = customer.get_phone_numbers()
            for call in data:
                if (call.src_number in phone_numbers) or \
                                    (call.dst_number in phone_numbers):
                    calls.append(call)
            return calls
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        """
        calls = []
        if (filter_string == '') or (filter_string[0] not in ['L', 'G']) or \
                (not filter_string[1:].isdigit()):
            return data
        else:
...
def is_coordinate(filter_string: str) -> bool:
    """Return True if the filter string is valid;
    ...
    """
    lower_left_coordinates = "-79.697878, 43.576959".split(', ')
    upper_right_coordinates = "-79.196382, 43.799568".split(', ')
    filter_coordinates = filter_string.split(', ')
    if (filter_string == '') or (len(filter_coordinates) != 4):
        return False
    for coordinate in filter_coordinates:
        try:
            float(coordinate)
        except ValueError:
            return False
    if float(filter_coordinates[2]) < float(filter_coordinates[0]) \
            or float(filter_coordinates[3]) < float(filter_coordinates[1]):
        return False
    for coordinate in filter_coordinates[0::2]:
        if float(coordinate) < float(lower_left_coordinates[0]) or \
                float(coordinate) > float(upper_right_coordinates[0]):
            return False
    for coordinate in filter_coordinates[1::2]:
        if float(coordinate) < float(lower_left_coordinates[1]) or \
                float(coordinate) > float(upper_right_coordinates[1]):
            return False
    return True
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        customer_calls = []
        filter_coordinates = filter_string.split(', ')
        if is_coordinate(filter_string) is False:
            return data
        else:
            for call in data:
                if float(filter_coordinates[0]) <= call.dst_loc[0] <= \
                        float(filter_coordinates[2]):
                    if float(filter_coordinates[1]) <= call.dst_loc[1] <= \
                                                float(filter_coordinates[3]):
                        calls.append(call)
                if float(filter_coordinates[0]) <= call.src_loc[0] <= \
                        float(filter_coordinates[2]):
                    if float(filter_coordinates[1]) <= call.src_loc[1] <= \
                                                float(filter_coordinates[3]):
                        calls.append(call)
            for call in calls:
                if call not in customer_calls:
